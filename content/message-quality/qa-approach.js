// Document content for QA Approach
const qaApproachContent = {
    id: '1Dtq1IV2Fe_-nxPvdhEt2wdLwIny8OnUpWiCTyghQN7c',
    title: 'QA Approach',
    type: 'presentation',
    description: 'Quality assurance processes and methods',
    tags: ['quality', 'assurance', 'QA', 'standards', 'review'],
    content: ``
};

// Export for use in main search
window.qaApproachContent = qaApproachContent;

// Register with document registry
if (window.documentRegistry) {
    window.documentRegistry.registerDocument(qaApproachContent, 'Message Quality');
} else {
    window.addEventListener('load', () => {
        if (window.documentRegistry) {
            window.documentRegistry.registerDocument(qaApproachContent, 'Message Quality');
        }
    });
}
